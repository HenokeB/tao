...
//$this service getVariableData( core_kernel_classes_Resource $variable) to be used in here
	...
	$activityExecutions =array(); $activityExecutionDetailsMissing = false;
		...
		$activityExecutionDetailsMissing =true;
		$activityExecutions[] = $origin;
	    }
        }
	if ($activityExecutionDetailsMissing) {
	    // sort activityExecutionsUris
	    $sortedActivityExecutions = taoResults_helpers_UriSorter::sort(array_unique($activityExecutions));
	    //jSort(array_unique($activityExecutions));

	    foreach ($variables as $origin => $data) {
		$variables[$origin]['label'] = "at index #".array_search($origin,$sortedActivityExecutions);
		$variables[$origin]['itemModel'] = "Not found";        }
	}
