	...
	var responseClassFunctions = {
		...
		buildGrid : function(tableElementId, serverResponse) {
			...
			for (var i=0; i<serverResponse.colModel.length; i++) {
				...
				if(colElt.width) colModel[i].width = colElt.width;
			...
			var addfunc = function(){
				...
				if(response.areaMapping && !interaction.shapeEditor) return false;
		...
		validateCurrentResponseRow : function(){
			...
			if(repeatedChoice){
				util.confirmBox(__('Invalid Response Declaration'), __('There cannot be identical choice in a row.')); 
				return false;
			}
			if(repeatedRow){
				util.confirmBox(__('Invalid Response Declaration'), __('There is already a row with identical choices.')); 
				return false;
			}
			if(!maxChoicesRespected){
				util.confirmBox(__('Invalid Response Declaration'), __('Impossible to exceed the maximum number of choices defined in the interaction.')); 
				return false;
			}
		...
		editGridRow: function(rowId) {
			...
			if(id>=0 && id!=='' && id!==this.currentRowId){
				...
				$currentRow.find('input[name=score]').width('93%');
