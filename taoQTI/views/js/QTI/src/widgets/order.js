		...
		$(ctx.qti_item_id+" .qti_graphic_order_spotlist li").each(function(){
			...
			switch(currentHotSpotShape){
				...
				case "ellipse":
					...
					break;
				case "poly":
					var polyCoords=QTIWidgetFactory.polyCoordonates(ctx.opts.graphicOrderChoices[$(this).attr("id")]["coords"]);
			...
			if(currentShape){
				...
				$(currentShape.node).bind("mousedown",{	
						...
						if (state_obj[e.data.name].state == "empty" && elementSelected==1) {
							...
							var coords = QTIWidgetFactory.getShapeCenter(e.data.zis);
							var orderInfo 	= paper.text(coords.x, coords.y, $(ctx.qti_item_id+" .pickup_area li.selected").text());
							var orderInfo1 	= paper.text(coords.x, coords.y, $(ctx.qti_item_id+" .pickup_area li.selected").text());
							...
							QTIWidgetFactory.setDeactivatedShapeAttr(e.data.zis, 400);
