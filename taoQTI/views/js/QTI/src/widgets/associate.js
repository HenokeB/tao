...
QTIWidget.graphic_associate = function (ctx){
	...
	function createArea(imageWidth, imageHeight){
		...
		var $raphaelCanvas = $(ctx.qti_item_id+' .svg-container');
		var paper = Raphael($raphaelCanvas[0], imageWidth, imageHeight);
		...
		$(ctx.qti_item_id+" .qti_graphic_associate_spotlist li").each(function(){
			...
			switch(currentHotSpotShape){
				...
				break;
				case "poly":
					polyCoords=QTIWidgetFactory.polyCoordonates(ctx.opts.graphicAssociateChoices[$(this).attr("id")]["coords"]);
					currentShape=paper["path"](polyCoords);
					
					if (ctx.graphicDebug) currentShape.attr("stroke-width", "3px");
					
					var polyCenter = QTIWidgetFactory.getShapeCenter(currentShape);
			...
			for (var i in line_ref_obj){
					line_ref_obj[i].attr("stroke","black").attr('stroke-width', '4');
			}
			...
			if (e.data.pair.length<1){
				...
				$(line.node).on("mousedown",{
					...
					$(deleteButton.node).off("mousedown").on("mousedown",{zisbutton:deleteButton}, function(e){
						e.preventDefault();
		...
		function associate(hotspot1, hotspot2){
			if(model_obj[hotspot1] && model_obj[hotspot2]){
				var startShape = model_obj[hotspot1].ref;
							if(startShape){
								$(startShape.node).trigger("mousedown",{
									zis     : startShape,
						name	: hotspot1,
									pair    : pointsPair,
						zepointer: model_obj[hotspot1].pointer
								});
						
					var endShape = model_obj[hotspot2].ref;
								if(endShape){
									$(endShape.node).trigger("mousedown",{
										zis		: endShape,
							name		: hotspot2,
										pair		: pointsPair,
							zepointer	: model_obj[hotspot2].pointer
									});
								}
							}
						}
		...
		if(ctx.opts["values"]){
			...
			if(typeof(values) == 'object'){
				if(values.length == 2){
					associate(values[0], values[1]);
				}
				for(var index in values){
					var value = values[index];
					if(typeof(value) == 'object'){
						if(value.length == 2){
							associate(value[0], value[1]);
			}
		}
				}
			}
