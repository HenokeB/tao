 ...
 * @methodOf QTIWidget
 * @param {Object} ctx the QTIWidget context
 * @param {Object} resultCollector
 */
QTIWidget.graphic_gap_match = function (ctx, resultCollector){
	
	$(ctx.qti_item_id +' ul.choice_list').hide();
	...
	if(isNaN(imageHeight)|| isNaN(imageWidth)){
		...
		$loadedImg.load(function(){
			...
			shapes = createArea(imageWidth, imageHeight);
		});
	...
	function createArea(imageWidth, imageHeight){
		...
		var shapes = {};
		for(var identifier in ctx.opts.matchMaxes){
				...
				switch(ctx.opts.matchMaxes[identifier]['shape']){
					...
					case "poly":
						shapes[identifier] = paper.path(
								QTIWidgetFactory.polyCoordonates(ctx.opts.matchMaxes[identifier]["coords"])
							);			
					break;
				}
				
				if(shapes[identifier]){
					shapes[identifier].toFront();
					QTIWidgetFactory.setDeactivatedShapeAttr(shapes[identifier]);
		...
		$(ctx.qti_item_id+' .svg-container').droppable({
			...
			drop: function(event, ui){
				...
				if(result.length > 0) {
					return fillGap($(this), $(ui.draggable), result[0][2].id, result[0][2]);
				}
			}
		});
	...
	var fillGap = function($jDropped, $jDragged, gapId, raphShape){
		...
		if(_matchGroup.length > 0){
			if($.inArray(gapId, _matchGroup) < 0){
				QTIWidgetFactory.animateForbiddenShape(raphShape);
				return false;
			}
		}
		...
		var _hotspotMatchMax = parseInt(ctx.opts.matchMaxes[gapId].matchMax);
		var _hotspotCurrent = parseInt(ctx.opts.matchMaxes[gapId].current);
		_hotspotCurrent = isNaN(_hotspotCurrent)?0:_hotspotCurrent;
		if (_hotspotMatchMax == 0 || _hotspotCurrent < _hotspotMatchMax) {
			//ok, allow dropping
			ctx.opts.matchMaxes[gapId].current = parseInt(_hotspotCurrent+1);
		}else{
			QTIWidgetFactory.animateForbiddenShape(raphShape);
			return false
		}
		...
		//ok, set response : 
		QTIWidgetFactory.animateAllowedShape(raphShape);
		var responseId = resultCollector.append({'hotspot':gapId,'gapImg':draggedId});
		
		//check the matchMax of the element
		var _gapImgMatchMax = parseInt(ctx.opts.matchMaxes[draggedId].matchMax);
		var _gapImgCurrent 	= parseInt(ctx.opts.matchMaxes[draggedId].current);
		_gapImgCurrent = isNaN(_gapImgCurrent)?0:_gapImgCurrent;
		if (_gapImgMatchMax == 0 || _gapImgCurrent < _gapImgMatchMax) {
			_gapImgCurrent++;
			ctx.opts["matchMaxes"][draggedId].current = _gapImgCurrent;
			if(_gapImgMatchMax > 0 && _gapImgCurrent >= _gapImgMatchMax){
				$jDragged.hide();
			}
		}
		...
		var gapCenter = QTIWidgetFactory.getShapeCenter(raphShape);
		...
		$(ctx.qti_item_id+' #'+filledId).draggable({
			...
			stop: function(){
				...
				removeFilledGap($(this),gapId,draggedId);
			},
			revert: false,
			containment: ctx.qti_item_id,
			cursor:"move"
	 ...
	 * @param {jQuery} jElement
	 */
	var removeFilledGap = function($jElement, gapId, draggedId){
		...
		if(gapId.length){
			var _matchMax = Number(ctx.opts.matchMaxes[gapId].matchMax);
			var _current = Number(ctx.opts.matchMaxes[gapId].current);
			if (_current > 0) _current--;
			ctx.opts.matchMaxes[gapId].current = _current;
		}
	};
		...
		if(typeof(values) == 'object'){
			for (var i in values){
				...
				if( value['0'] && value['1']){
					var gap = value['0'];
					var choice = value['1'];
					fillGap($(ctx.qti_item_id+' .svg-container'), $(ctx.qti_item_id+" ul.qti_graphic_gap_match_spotlist li#"+choice+" > div"), gap, shapes[gap]);
				}
			}
		}
	}
