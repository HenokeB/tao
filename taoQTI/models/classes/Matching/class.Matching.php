 ...
 * @author Joel Bout, <joel.bout@tudor.lu>
 */
require_once('taoQTI/models/classes/Matching/class.AreaMap.php');
 ...
 * @author Joel Bout, <joel.bout@tudor.lu>
 */
require_once('taoQTI/models/classes/Matching/class.ExitResponseException.php');
 ...
 * @author Joel Bout, <joel.bout@tudor.lu>
 */
require_once('taoQTI/models/classes/Matching/class.Map.php');
 ...
 * @author Joel Bout, <joel.bout@tudor.lu>
 */
require_once('taoQTI/models/classes/Matching/class.VariableFactory.php');
 ...
 * @subpackage models_classes_Matching
 */
class taoQTI_models_classes_Matching_Matching
{
    // --- ASSOCIATIONS ---
    ...
    public function __construct()
    {
		...
		taoQTI_models_classes_Matching_Matching::$whiteFunctionsList = array (
    ...
    public static function parseExpressionRule($matches)
    {
        ...
        if (isset(taoQTI_models_classes_Matching_Matching::$whiteFunctionsList[$matches[1]])){
            $whiteListedFunction = taoQTI_models_classes_Matching_Matching::$whiteFunctionsList[$matches[1]];
        }
        ...
        if (isset($whiteListedFunction))
        {
            ...
            // Check if the function is present in the matching engine functions pool
                ...
                if (!method_exists ('taoQTI_models_classes_Matching_Matching', $functionName)) {
                    ...
                    if (isset($whiteListedFunction['mappedFunction'])){
                        ...
                        if (!method_exists ('taoQTI_models_classes_Matching_Matching', $whiteListedFunction['mappedFunction'])) {
                            throw new Exception ('taoQTI_models_classes_Matching_Matching::parseExpressionRule an error occured, the expression ['. $functionName .'] has been mapped to ['. $whiteListedFunction['mappedFunction'] .'] but is not yet instantiated');
                        }
        ...
        {
            throw new Exception ('taoQTI_models_classes_Matching_Matching::parseExpressionRule an error occured, the expression ['. $functionName .'] is unknown ');
        }
    ...
    public function evaluate()
    {
        ...
        try {
			eval ($this->getRule());
        } catch (taoQTI_models_classes_Matching_ExitResponseException $e) {
    ...
    public function setCorrects($data)
    {
    	...
    	if (gettype($data) != 'array')
    		throw new Exception ('taoQTI_models_classes_Matching_Matching::setCorrects is waiting on an array, a '.gettype($data).' is given');

		foreach ($data as $key=>$correct){
			try {
				$var = taoQTI_models_classes_Matching_VariableFactory::create ($correct->value);
				
				if (isset ($this->corrects[$correct->identifier]))
					throw new Exception ('taoQTI_models_classes_Matching_Matching::setCorrects a correct variable with the identifier '.$correct->identifier.' exists yet');
    ...
    public function setAreaMaps(   $data)
    {
        ...
        if (gettype($data) != 'array'){
            throw new Exception ('taoQTI_models_classes_Matching_Matching::setAreaMaps is waiting on an array, a '.gettype($data).' is given. '.$data);
        }

        foreach ($data as $key=>$map){
            try {
                $var =  new taoQTI_models_classes_Matching_AreaMap ($map);
                
                if (isset ($this->areaMaps[$map->identifier]))
                    throw new Exception ('taoQTI_models_classes_Matching_Matching::setMaps a map variable with the identifier '.$map->identifier.' exists yet');
    ...
    public function setMaps(   $data)
    {
        ...
        if (gettype($data) != 'array'){
    		throw new Exception ('taoQTI_models_classes_Matching_Matching::setMaps is waiting on an array, a '.gettype($data).' is given. '.$data);
        }

		foreach ($data as $key=>$map){
			try {
				$var =  new taoQTI_models_classes_Matching_Map ($map);
    ...
    public function setOutcomes($data)
    {
        ...
        if (gettype($data) != 'array'){
    		throw new Exception ('taoQTI_models_classes_Matching_Matching::setOutcomes is waiting on an array, a '.gettype($data).' is given. '.$data);
        }
		...
		foreach ($data as $key=>$outcome){
			try {
				...
				$var = taoQTI_models_classes_Matching_VariableFactory::create ($outcomeDefaultValue, $outcomeDefinedType);
				
				if (isset ($this->responses[$outcome->identifier]))
					throw new Exception ('taoQTI_models_classes_Matching_Matching::setReponses a correct variable with the identifier '.$outcome->identifier.' exists yet');
    ...
    public function setResponses($data)
    {
        ...
        if (gettype($data) != 'array'){
    		throw new Exception ('taoQTI_models_classes_Matching_Matching::setReponses is waiting on an array, a '.gettype($data).' is given. '.$data);
        }
		
		foreach ($data as $key=>$response){
			try {
				$var = taoQTI_models_classes_Matching_VariableFactory::create ($response->value);
				if (isset ($this->responses[$response->identifier]))
					throw new common_Exception('taoQTI_models_classes_Matching_Matching::setReponses a response variable with the identifier '.$response->identifier.' exists already');
    ...
    public function setRule($rule)
    {
        ...
        //implode ('|', array_keys(taoQTI_models_classes_Matching_Matching::$whiteFunctionsList))
    	
    	$this->rule = preg_replace_callback (
				'/([a-zA-Z_\-1-9]*)[\s]*\(/'
				, 'taoQTI_models_classes_Matching_Matching::parseExpressionRule'
				, $rule
			).';';
    ...
    public function andExpression($options)
    {
        ...
        for ($i = 1; $i < func_num_args(); ++$i) {
            ...
            $matchingSubExp = taoQTI_models_classes_Matching_VariableFactory::toBooleanBaseType ($subExp);
            
            if ($matchingSubExp == null) {
                throw new Exception ("TtaoQTI_models_classes_Matching_Matching::or an error occured : The expression passed [".$matchingSubExp."] to the operator has to be a valid boolean expression with single cardinality");
            } else {
    ...
    public function createVariable($options)
    {
        ...
        if (!isset($options['type'])) {
            $returnValue = taoQTI_models_classes_Matching_VariableFactory::create (func_get_arg(1));
        }
        else 
        {
            switch ($options['type']){
                ...
                case 'tuple':
                    ...
                    $returnValue = taoQTI_models_classes_Matching_VariableFactory::create ((object)$values);
                    break;
                ...
                case 'list':
                    ...
                    $returnValue = taoQTI_models_classes_Matching_VariableFactory::create ($values);
                    break;
                
                // Type unknown, throw an Exception
                case 'default':
                    throw new Exception ('taoQTI_models_classes_Matching_Matching::createVariable : type unknown ['.$options['type'].']');
            }  
        }
    ...
    public function contains(   $options,    $expr1,    $expr2)
    {
        ...
        if (!($expr1 instanceOf taoQTI_models_classes_Matching_Collection)) {
            throw new Exception ("TtaoQTI_models_classes_Matching_Matching::contains an error occured : The operator requires as first argument an expression of type Collection");
        }
    ...
    public function divide(   $options,    $expr1,    $expr2)
    {
        ...
        $matchingExpr1 = taoQTI_models_classes_Matching_VariableFactory::toNumericBaseType ($expr1);
        // IF the first expression is not a numerical base type
        if ($matchingExpr1 == null) {
        	$expr1 = is_array($expr1) ? implode('-', $expr1) : $expr1;
            throw new Exception ("TtaoQTI_models_classes_Matching_Matching::divide an error occured : The first expression passed [".$expr1."] to the operator has to be a valid numerical expression with single cardinality");
        }

        $matchingExpr2 = taoQTI_models_classes_Matching_VariableFactory::toNumericBaseType ($expr2);
        // IF the second expression is not a numerical base type
        if ($matchingExpr2 == null) {
        	$expr2 = is_array($expr2) ? implode('-', $expr2) : $expr2;
            throw new Exception ("TtaoQTI_models_classes_Matching_Matching::divide an error occured : The second expression passed [".$expr2."] to the operator has to be a valid numerical expression with single cardinality");
        }
    ...
    public function equal($options,    $expr1,    $expr2)
    {
        ...
        if ($expr1 instanceof taoQTI_models_classes_Matching_BaseTypeVariable){
            $expr1Value = $expr1->getValue();
        }
        // The first expression is not a scalar
        else if (!is_scalar ($expr1)) {
            throw new Exception('taoQTI_models_classes_Matching_Matching::equal : the first argument must be a scalar');
        }
        else {
            $expr1Value = $expr1;
        }
        ...
        if ($expr2 instanceof taoQTI_models_classes_Matching_BaseTypeVariable){
            $expr2Value = $expr2->getValue();
        }
        // The second expr is not a scalar
        else if (!is_scalar($expr2)) {
            throw new Error('taoQTI_models_classes_Matching_Matching::equal : the second argument must be a scalar');
        }
        else {
            $expr2Value = $expr2;
        }
    ...
    public function exitResponse()
    {
		...
		throw new taoQTI_models_classes_Matching_ExitResponseException();
    ...
    public function gt(   $options,    $expr1,    $expr2)
    {
        ...
        if ($matchingExpr1 == null) {
        	$expr1 = is_array($expr1) ? implode('-', $expr1) : $expr1;
            throw new Exception ("taoQTI_models_classes_Matching_Matching::gt an error occured : The first expression passed [".$expr1."] to the operator has to be a valid numerical expression with single cardinality");
        }
        ...
        if ($matchingExpr2 == null) {
        	$expr2 = is_array($expr2) ? implode('-', $expr2) : $expr2;
            throw new Exception ("taoQTI_models_classes_Matching_Matching::gt an error occured : The second expression passed [".$expr2."] to the operator has to be a valid numerical expression with single cardinality");
        }
     ...
     * @return boolean
     */
    public function isNull($options,  taoQTI_models_classes_Matching_Variable $var)
    {
        $returnValue = (bool) false;
    ...
    public function lt(   $options,    $expr1,    $expr2)
    {
        ...
        // IF the first expression is not a numerical base type
        if ($matchingExpr1 == null) {
            throw new Exception ("taoQTI_models_classes_Matching_Matching::lt an error occured : The first expression passed [".$expr1."] to the operator has to be a valid numerical expression with single cardinality");
        }

        $matchingExpr2 = taoQTI_models_classes_Matching_VariableFactory::toNumericBaseType ($expr2);
        // IF the second expression is not a numerical base type
        if ($matchingExpr2 == null) {
            throw new Exception ("taoQTI_models_classes_Matching_Matching::lt an error occured : The second expression passed [".$expr2."] to the operator has to be a valid numerical expression with single cardinality");
        }
     ...
     * @return double
     */
    public function mapResponse($options,  taoQTI_models_classes_Matching_Map $map,  taoQTI_models_classes_Matching_Variable $expr)
    {
        ...
        if (!isset($map)){
            throw new Exception("taoQTI_models_classes_Matching_Matching::mapResponse error : the first argument [taoQTI_models_classes_Matching_Variable] does not exist");
        }
        else if (!isset($expr)){
            throw new Exception("taoQTI_models_classes_Matching_Matching::mapResponse error : the second argument [taoQTI_models_classes_Matching_Variable] does not exist");
        }
     ...
     * @return double
     */
    public function mapResponsePoint($options,  taoQTI_models_classes_Matching_AreaMap $map,  taoQTI_models_classes_Matching_Variable $expr)
    {
        $returnValue = (float) 0.0;
        ...
        if (!isset($map)){
            throw new common_Exception ("taoQTI_models_classes_Matching_Matching::mapResponsePoint error : the first argument [taoQTI_models_classes_Matching_AreaMap] does not exist");
        }
        else if (!isset($expr)){
            throw new common_Exception ("taoQTI_models_classes_Matching_Matching::mapResponsePoint error : the second argument [taoQTI_models_classes_Matching_Variable] does not exist");
        }
    ...
    public function not(   $options,    $expr)
    {
        ...
        $matchingExpr = taoQTI_models_classes_Matching_VariableFactory::toBooleanBaseType ($expr);
        if ($matchingExpr == null){
        	$expr = is_array($expr) ? implode('-', $expr) : $expr;
            throw new Exception ("taoQTI_models_classes_Matching_Matching::not an error occured : The expression passed [".$expr."] to the operator has to be a valid boolean expression with single cardinality");
        } else {
    ...
    public function orExpression(   $options)
    {
        ...
        for ($i = 1; $i < func_num_args(); ++$i) {
            ...
            if ($matchingSubExp == null) {
                throw new Exception ("taoQTI_models_classes_Matching_Matching::or an error occured : The expression passed [".$matchingSubExp."] to the operator has to be a valid boolean expression with single cardinality");
            } else {
                if ($matchingSubExp->isNull()){
                    $returnValue = null;
                    break;
    ...
    public function product(   $options)
    {
        ...
        for ($i = 1; $i < func_num_args(); ++$i) {
            ...
            $matchingSubExp = taoQTI_models_classes_Matching_VariableFactory::toNumericBaseType ($subExp);
            // If the sub expression is not a numerical base type variable
            if ($matchingSubExp == null){
            	$subExp = is_array($subExp) ? implode('-', $subExp) : $subExp;
                throw new Exception ("taoQTI_models_classes_Matching_Matching::product an error occured : The [".$i."] expression passed [".$subExp."] to the operator has to be a valid numerical expression with single cardinality");
            } 
    ...
    public function round(   $options,    $expr)
    {
        ...
        if ($expr == null) {
            ...
            $matchingExpr = taoQTI_models_classes_Matching_VariableFactory::toNumericBaseType ($expr);
            // IF the expression is not a numerical base type
            if ($matchingExpr == null) {
            	$expr = is_array($expr) ? implode('', $expr) : $expr;
                throw new Exception ("taoQTI_models_classes_Matching_Matching::round an error occured : The expression passed [".$expr."] to the operator has to be a valid numerical expression with single cardinality");
            }else{
    ...
    public function setOutcomeValue($id,    $value)
    {
        ...
        if($outcome == null){
            throw new Exception ('taoQTI_models_classes_Matching_Matching::setOutcomeValue error : the outcome value '.$id.' does not exist');
        }
        if ($value instanceof taoQTI_models_classes_Matching_Variable){
            $outcome->setValue ($value->getValue());
        }
        else {
            //if (taoQTI_models_classes_Matching_BaseTypeVariable::isValidValue ($value)){
                $outcome->setValue ($value);
            //}else{
            //    throw new Exception ('taoQTI_models_classes_Matching_Matching::setOutcomeValue error : unable to set a value of this type ['.gettype($value).']');
            //}
        }
    ...
    public function subtract(   $options,    $expr1,    $expr2)
    {
        ...
        if ($matchingExpr1 == null) {
        	$expr1 = is_array($expr1) ? implode('-', $expr1) : $expr1;
            throw new Exception ("taoQTI_models_classes_Matching_Matching::substract an error occured : The first expression passed [".$expr1."] to the operator has to be a valid numerical expression with single cardinality");
        }
        ...
        if ($matchingExpr2 == null) {
        	$expr2 = is_array($expr2) ? implode('-', $expr2) : $expr2;
            throw new Exception ("taoQTI_models_classes_Matching_Matching::substract an error occured : The second expression passed [".$expr2."] to the operator has to be a valid numerical expression with single cardinality");
        }
    ...
    public function sum(   $options)
    {
        ...
        for ($i = 1; $i < func_num_args(); ++$i) {
            ...
            if ($matchingSubExp == null){
            	$subExp = is_array($subExp) ? implode('-', $subExp) : $subExp;
                throw new Exception ("taoQTI_models_classes_Matching_Matching::sum an error occured : The [".$i."] expression passed [".$subExp."] to the operator has to be a valid numerical expression with single cardinality");
            } 
            // If the sub expression value is null
            else if ($matchingSubExp->isNull()){
                $returnValue = null;
...
} /* end of class taoQTI_models_classes_Matching_Matching */
