 ...
 * @subpackage models_classes_Matching
 */
class taoQTI_models_classes_Matching_Tuple
    extends taoQTI_models_classes_Matching_Collection
{
     ...
     * @return boolean
     */
    public function match( taoQTI_models_classes_Matching_Variable $tuple)
    {
        ...
        if ($this->length() != $tuple->length()){
	        ...
	        foreach ($this->value as $key=>$elt){
	        	...
	        	if ($compareElt == null){
	        		...
	        		throw new Exception ('taoQTI_models_classes_Matching_Tuple::match an error occured : types of the elements to match are not the same ['.$elt->getType ().'] and ['.$compareElt->getType().']');
	        		$returnValue = false;
	        		break;
...
} /* end of class taoQTI_models_classes_Matching_Tuple */
