 ...
 * @author Jerome Bogaerts, <jerome.bogaerts@tudor.lu>
 */
require_once('taoQTI/models/classes/Matching/class.Collection.php');
 ...
 * @subpackage models_classes_Matching
 */
class taoQTI_models_classes_Matching_List
    extends taoQTI_models_classes_Matching_Collection
{
    // --- ASSOCIATIONS ---
     ...
     * @see Variable
     */
    public function match( taoQTI_models_classes_Matching_Variable $list)
    {
        ...
        for ($i=0; $i<$this->length(); $i++) {
	        ...
	        for ($j=0; $j<$list->length(); $j++) {
	        	if ($this->getElement($i)->getType () != $list->getElement($j)->getType()){
	        		...
	        		throw new Exception('taoQTI_models_classes_Matching_List::match an error occured : types of the elements to match are not the same ['. $this->getElement($i)->getType () .'] and ['. $list->getElement($j)->getType() .']');
	        		$returnValue = false;
	        	} 
    ...
    public function setValue(   $data)
    {
    	...
    	foreach ($data as $key=>$elt){
    	    if ($elt instanceOf taoQTI_models_classes_Matching_Variable) {
                $this->value[$key] = $elt;
            } else {
                $this->value[$key] = taoQTI_models_classes_Matching_VariableFactory::create ($elt);
            }
    	}
...
} /* end of class taoQTI_models_classes_Matching_List */
