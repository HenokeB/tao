    ...
    extends tao_models_classes_ClassService
{
    ...
    public function generateProcess( core_kernel_classes_Resource $delivery)
    {
		...
		if(!is_null($deliveryProcess)){
			...
			//specific rule for the Delivery Process, the process i snot deleted if there are processExecutions.
			$processExecutionsClass = new core_kernel_classes_Class(CLASS_PROCESSINSTANCES);
			$processExecutions = $processExecutionsClass->searchInstances(array(PROPERTY_PROCESSINSTANCES_EXECUTIONOF => $oldDeliveryProcess));
			if ((count($processExecutions)==0)){
				$authoringService = taoDelivery_models_classes_DeliveryAuthoringService::singleton();
